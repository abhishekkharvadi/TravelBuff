...
import { 
  ...
  Map, Edit, CheckSquare, X, DollarSign, RefreshCw 
} from 'lucide-react';
...
export default function TripPlanning({ token }) {
  ...
  const handleAddMainReservation = async (e) => {
    ...
    if (!isNaN(parsedCost) && parsedCost > 0) {
      const newExp = {
        id: generateUUID(),
        trip_id: selectedTrip.id,
        date: selectedTrip.start_date || new Date().toISOString().split('T')[0],
        amount: parsedCost,
        currency: baseCurrency,
        category: 'Transportation',
  ...
  const handleAddExpense = async (e) => {
    e.preventDefault();
    if (!expAmount || !expDate || !selectedTrip) return;
            ...
            {dayReservations.map(res => (
              <div key={res.id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-glass)', borderRadius: 'var(--radius-sm)', padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  {!tripModeActive && (
                    <button className="photo-action-btn" onClick={() => queueSyncAction('reservations', 'delete', { id: res.id })}>
                      <Trash2 size={12} />
                    </button>
                  )}
                </div>
  ...
  const tripExpensesList = selectedTrip ? expenses.filter(e => e.trip_id === selectedTrip.id) : [];
            ...
            <div style={{ padding: '0 16px 16px 16px', borderTop: '1px solid rgba(255,255,255,0.02)' }}>
              <button 
                ...
                style={{ 
                  ...
                  padding: '6px 12px', 
                  fontSize: '0.75rem', 
                  background: activeTripId === trip.id ? 'var(--accent-primary)' : 'rgba(255,255,255,0.05)',
                  borderColor: activeTripId === trip.id ? 'var(--accent-primary)' : 'var(--border-glass)',
                  color: activeTripId === trip.id ? '#000' : '#fff',
                  width: 'auto',
                  fontWeight: 600
                }}
              >
                {activeTripId === trip.id ? '⭐ Active Trip' : 'Set as Active'}
              </button>
            </div>
          </div>
        ))}
            ...
            <div className="dialog-body" style={{ flexGrow: 1, padding: '24px', maxHeight: 'none', overflowY: 'visible' }}>
              ...
              {!tripModeActive && (
                <div className={!printOptExpenses ? 'no-print' : ''} style={{ background: '#1c1b22', padding: '24px', borderRadius: 'var(--radius-md)', marginBottom: '24px', border: '1px solid var(--border-glass)' }}>
                  <h3 style={{ marginBottom: '16px', marginTop: 0 }}>Expenses list & Distribution</h3>
                      ...
                      <tbody>
                        {tripExpensesList.map(exp => (
                          <tr key={exp.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                            <td style={{ padding: '8px' }}>
                              <span className="tag-badge" style={{ backgroundColor: 'rgba(139,92,246,0.1)', color: 'var(--accent-primary-hover)', fontSize: '0.65rem' }}>
                                {exp.category}
                              </span>
                            </td>
                            <td style={{ padding: '8px', color: 'var(--text-secondary)' }}>{exp.notes}</td>
                            <td style={{ padding: '8px', fontWeight: 600 }}>
                              {exp.amount} {exp.currency} 
                              {exp.currency !== baseCurrency && ` (~${getConvertedAmount(exp.amount, exp.currency).toFixed(2)} ${baseCurrency})`}
                            </td>
                            <td style={{ padding: '8px', color: 'var(--text-secondary)' }}>{exp.date}</td>
                            <td style={{ padding: '8px' }}>
                              <button className="photo-action-btn" onClick={() => queueSyncAction('expenses', 'delete', { id: exp.id })}>
                                <Trash2 size={12} />
                              </button>
                            </td>
                          </tr>
                ...
                <div className="no-print" style={{ background: '#1c1b22', padding: '24px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-glass)', marginBottom: '24px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    <div style={{ flex: '1 1 200px' }}>
                      ...
                      <select id="itin-date-select" className="form-control">
                        {tripDates.map(date => (
                          <option key={date} value={date}>{date}</option>
                        ))}
                      </select>
                    ...
                    <div style={{ flex: '1 1 100%', marginTop: '12px' }}>
                      ...
                      <div style={{ maxHeight: '150px', overflowY: 'auto', border: '1px solid var(--border-glass)', padding: '8px', borderRadius: '4px', background: '#121217', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        {places.filter(p => {
                          // Filter out places that are already present in ANY date of ANY itinerary or trip
                          const isAlreadyAdded = itineraries.some(item => item.place_id === p.id);
                  ...
                  <button 
                    onClick={async () => {
                      const date = document.getElementById('itin-date-select').value;
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px' }}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                <input 
                  ...
                  style={{ width: '16px', height: '16px', accentColor: 'var(--accent-primary)' }}
                />
                <span style={{ color: '#fff', fontSize: '0.9rem' }}>Expenses List & Distribution</span>
              </label>
            </div>
